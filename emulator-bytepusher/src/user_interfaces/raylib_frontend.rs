...
impl RaylibFrontend
{
    ...
    {
        let mut result_internals = raylib::init().size(768, 768).title("remu BytePusher").build();

        let mut result = Self {
            output: result_internals.0.load_render_texture(&result_internals.1, 256, 256).unwrap(),
    ...
    pub fn update(&mut self, emulator: &mut Emulator, delta: f64)
    {
        ...
        for y in 0..emulator.get_display_height()
        {
            for x in 0..emulator.get_display_width()
            {
                ...
                output_context.draw_pixel(
                    x as i32, (emulator.get_display_height() - (y + 1)) as i32,
                    Color::from((((pixel & 0xFF0000) >> 16) as u8, ((pixel & 0xFF00) >> 8) as u8, (pixel & 0xFF) as u8, 0xFF as u8))
                );
            }
        }
